...
if ds_file and sac_file:
    ...
    # 5. 그래프 레이아웃 설정
    fig.update_layout(
        title="실시간 다중 레이어 분석 차트 (마우스로 특정 구간을 드래그하여 확대/축소 가능)",
        xaxis_title="주행 시간 (초)",
    ...
    # 차선 변경 횟수 계산 방어 로직 추가
    lane_change_count = len(df_ds[df_ds[ds_lane_col].diff().abs() > 0]) if ds_lane_col in df_ds.columns else 0
    col3.metric("차선 변경 횟수", f"{lane_change_count} 회")
